    ...
    <BrokerLayout>
      <div style={{ maxWidth: 760 }}>
        <div style={{ marginBottom: 48 }}>
          <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: "0.75rem", letterSpacing: "0.12em", textTransform: "uppercase", color: "rgba(201,168,112,0.7)", marginBottom: 10 }}>{today}</div>
          ...
          <p style={{ fontSize: "0.9rem", color: "rgba(242,239,233,0.6)", letterSpacing: "0.01em" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 40 }}>
          {[
            ...
            <div key={m.label} style={{ background: "#0E0C09", border: "1px solid rgba(201,168,112,0.08)", borderRadius: 12, padding: "20px 24px" }}>
              ...
              <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: "0.72rem", letterSpacing: "0.12em", textTransform: "uppercase", color: "rgba(242,239,233,0.55)" }}>{m.label}</div>
            </div>
          ))}
        </div>
        ...
        {!loading && prioritarios.length > 0 && (
          <div style={{ marginBottom: 32 }}>
            <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: "0.72rem", letterSpacing: "0.12em", textTransform: "uppercase", color: "rgba(201,168,112,0.7)", marginBottom: 12 }}>Requieren atención</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {prioritarios.slice(0, 5).map(e => (
                <div key={e.email?.id} style={{ background: "#0E0C09", border: "1px solid rgba(201,168,112,0.12)", borderLeft: "2px solid #C9A870", borderRadius: 3, padding: "14px 18px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
                    <div style={{ fontSize: "0.88rem", color: "#F2EFE9", fontWeight: 500, marginBottom: 3 }}>{e.email?.subject || "(sin asunto)"}</div>
                    <div style={{ fontSize: "0.8rem", color: "rgba(242,239,233,0.6)" }}>{e.email?.from_name || e.email?.from_email}</div>
                  </div>
                  <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: "0.72rem", letterSpacing: "0.12em", textTransform: "uppercase", color: "#C9A870", opacity: 0.7 }}>Prioritario</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {!loading && emails.length === 0 && (
          <div style={{ textAlign: "center", padding: "60px 0", color: "rgba(242,239,233,0.45)", fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: "0.78rem", letterSpacing: "0.12em", textTransform: "uppercase" }}>
            Sin mensajes hoy
          </div>
        )}
      </div>
