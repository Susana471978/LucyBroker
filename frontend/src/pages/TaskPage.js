...
const PRIORITY_STYLES = {
    high: { label: 'Alta', color: 'var(--champagne)', bg: 'rgba(201,178,124,0.1)', border: 'rgba(201,178,124,0.3)' },
    normal: { label: 'Normal', color: 'var(--text-tertiary)', bg: 'var(--border-subtle)', border: 'var(--border-subtle)' },
    low: { label: 'Baja', color: 'var(--text-tertiary)', bg: 'var(--surface-glass)', border: 'var(--border-subtle)' },
};
...
const TaskRow = ({ task, onToggle, onDelete }) => {
    ...
    return (
        ...
        ${task.done
                    ? 'bg-[linear-gradient(180deg,rgba(8,12,22,0.6)_0%,rgba(4,7,15,0.6)_100%)] border-[var(--border-subtle)]'
        ...
        >
            ...
            >
                {toggling ? (
                    <Loader2 className="w-5 h-5 text-[var(--champagne)] animate-spin" />
                ) : task.done ? (
          ...
          ${task.done ? 'line-through text-[var(--text-tertiary)]' : 'text-[var(--text-primary)]'}`}>
                    {task.title}
                </p>
                {task.notes && !task.done && (
                    <p className="text-xs text-[var(--text-tertiary)] mt-1 leading-relaxed">{task.notes}</p>
                )}
                <div className="flex items-center gap-2 mt-2 flex-wrap">
                    ...
                    {due && !task.done && (
                        <span className={`flex items-center gap-1 text-[10px] uppercase tracking-[0.08em] px-2 py-0.5 rounded-full
              ${due.urgent
                                ? 'text-[var(--champagne)] bg-[var(--glow-champagne)] border border-[var(--border-champagne)]'
                                : 'text-[var(--text-tertiary)] bg-[var(--border-subtle)] border border-[var(--border-subtle)]'
                            }`}>
          ...
          text-[var(--text-tertiary)] hover:text-[rgba(255,100,100,0.6)] disabled:opacity-30"
            >
    ...
    return (
        ...
        >
            ...
            <div className="flex items-center justify-between mb-4">
                <p className="text-xs text-[var(--champagne)] uppercase tracking-[0.1em]">Nueva tarea</p>
          ...
          rounded-[16px] px-4 py-3 text-sm text-[var(--text-primary)]
          placeholder:text-[var(--text-tertiary)] outline-none mb-3
          ...
          rounded-[16px] px-4 py-3 text-sm text-[var(--text-secondary)]
          placeholder:text-[var(--text-tertiary)] outline-none resize-none mb-3
          ...
          focus:border-[rgba(201,178,124,0.5)] focus:shadow-[0_0_0_1px_rgba(201,178,124,0.08)_inset,0_0_10px_rgba(201,178,124,0.10)] transition-all duration-300"
            ...
            <div className="flex items-center gap-3 mb-4 flex-wrap">
                ...
                ${priority === p
                                    ? 'bg-[var(--champagne-faint)] text-[var(--champagne)] border border-[rgba(201,178,124,0.3)] shadow-[0_0_10px_rgba(201,178,124,0.06)]'
                                    : 'text-[var(--text-tertiary)] border border-[var(--border-subtle)] hover:border-[rgba(88,160,255,0.28)]'
                                }`}>
            ...
            rounded-[16px] px-3 py-1 text-xs text-[var(--text-secondary)] outline-none
            shadow-[0_0_0_1px_rgba(90,170,255,0.04)_inset]
        ...
        <Layout>
            <div className="max-w-3xl mx-auto px-6 py-14 space-y-10">
                ...
                <div className="flex items-center justify-between">
                    ...
                    >
                        <p className="text-xs text-[var(--text-tertiary)] uppercase tracking-[0.1em] font-medium mb-3">
              ...
              ${showForm
                                ? 'bg-[var(--champagne-faint)] text-[var(--champagne)] border-[rgba(201,178,124,0.3)] shadow-[0_0_18px_rgba(201,178,124,0.06)]'
                                : 'bg-[linear-gradient(180deg,rgba(7,12,24,0.92)_0%,rgba(4,8,18,0.88)_100%)] text-[var(--text-secondary)] border-[rgba(88,160,255,0.24)] shadow-[0_0_0_1px_rgba(90,170,255,0.08)_inset,0_0_10px_rgba(54,126,255,0.06)] hover:border-[rgba(201,178,124,0.72)] hover:bg-[linear-gradient(180deg,rgba(24,18,10,0.96)_0%,rgba(14,10,6,0.92)_100%)] hover:text-[var(--champagne)] hover:shadow-[0_0_0_1px_rgba(201,178,124,0.12)_inset,0_0_14px_rgba(201,178,124,0.18)]'
                            }`}
                    >
                ...
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-20 gap-4">
                        ...
                        <p className="text-xs text-[var(--text-tertiary)] uppercase tracking-[0.1em]">Cargando tareas…</p>
                    </div>
                ) : (
                    <>
                        <AnimatePresence>
                            {pending.length === 0 && !showForm && (
                                ...
                                >
                                    ...
                                    <p className="text-sm text-[var(--text-tertiary)]">Todo al día</p>
                                    <p className="text-xs text-[var(--text-tertiary)]">No hay tareas pendientes</p>
                                </motion.div>
                            )}
                        </AnimatePresence>
                    ...
                    hover:text-[var(--text-secondary)] transition-colors mb-3 uppercase tracking-[0.1em]"
                                >
                                    <CheckCircle2 className="w-3.5 h-3.5" />
                                    {done.length} completada{done.length !== 1 ? 's' : ''}
                                    <span className="text-[var(--text-tertiary)]">{showDone ? '▲' : '▼'}</span>
                                </button>
