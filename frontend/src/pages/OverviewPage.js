  ...
  const fetchData = async () => {
    try {
      const [emailsRes, statsRes] = await Promise.all([
        ...
        axios.get(`${API}/emails/stats/summary`)
      ]);
  ...
  const handleAiSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.action?.type === 'filter') {
        ...
        if (response.data.action.payload.label) {
          params.set('label', response.data.action.payload.label);
        }
        if (response.data.action.payload.has_attachments) {
    ...
    <Layout>
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <motion.div
          ...
          className="glass-premium rounded-2xl p-6 mb-8 halo-card-ia ai-card"
        ...
        >
          ...
          <form onSubmit={handleAiSubmit} className="flex gap-3">
            <Input
              ...
              data-testid="overview-ai-input"
            />
            <Button
              type="submit"
              disabled={aiLoading}
              className="bg-blue-600 hover:bg-blue-500 text-white px-4"
              data-testid="overview-ai-submit"
            >
              {aiLoading ? (
          ...
          {aiResponse && (
            <motion.div
              ...
              data-testid="overview-ai-response"
            >
        ...
        {/* Priority Items or Silence Mode */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          ...
          {loading ? (
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl font-semibold text-slate-100">
                  {t(language, 'priorityItems')}
                </h2>
                <Button
                  variant="ghost"
                  onClick={() => navigate('/messages?label=PRIORITARIO')}
                  className="text-blue-400 hover:text-blue-300 hover:bg-blue-500/10"
                  data-testid="view-all-priority"
                >
                  Ver todos
              ...
              <div className="space-y-4">
                {priorityEmails.map((item, index) => (
                  <motion.div
                    ...
                    className="glass-subtle rounded-xl p-4 hover:bg-slate-800/60 cursor-pointer email-card"
                    onClick={() => navigate(`/messages?selected=${item.email.id}`)}
                    data-testid={`priority-item-${index}`}
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-2">
                          <span className={`px-2 py-0.5 rounded text-xs font-medium ${getPriorityClass(item.priority.priority_label)}`}>
                            {getPriorityLabel(item.priority.priority_label)}
                          </span>
                          {item.email.has_attachments && (
                            <Paperclip className="w-4 h-4 text-slate-500" strokeWidth={1.5} />
                          )}
                        </div>
                        ...
                        <p className="text-sm text-slate-400">
                          {item.email.from_name} • {new Date(item.email.date).toLocaleDateString()}
                        </p>
            ...
            <div className="glass-subtle rounded-2xl p-12 text-center silence-mode" data-testid="silence-mode">
              ...
              <h3 className="text-xl font-semibold text-slate-100 mb-2">
                {t(language, 'silenceMode')}
              </h3>
              <p className="text-slate-400">
                {t(language, 'silenceModeDesc')}
              </p>
            </div>
          )}
        </motion.div>
...
const StatCard = ({ icon, label, value, highlight, onClick, testId }) => (
  <motion.button
    ...
    whileTap={{ scale: 0.98 }}
    onClick={onClick}
    className={`glass-subtle rounded-xl p-4 text-left w-full ${
      highlight ? 'border-blue-500/30 halo-active' : ''
    }`}
    data-testid={testId}
  >
    <div className={`w-10 h-10 rounded-lg ${highlight ? 'bg-blue-500/20' : 'bg-slate-700/50'} flex items-center justify-center mb-3`}>
