...
import { Plus, Trash2, Flame, Check, X, ChevronDown } from 'lucide-react';
...
const StreakBadge = ({ streak }) => {
    ...
    return (
        <div className="flex items-center gap-1 text-xs">
            ...
            <span className="text-[rgba(201,178,124,0.6)] font-medium">{streak} días</span>
        </div>
    );
};
...
const HabitCard = ({ habit, onToggle, onDelete, onShowHistory, delay = 0 }) => {
    ...
    return (
        ...
        ${habit.completed_today
                    ...
                    : 'bg-[rgba(255,255,255,0.025)] border-[rgba(255,255,255,0.06)] hover:border-[rgba(255,255,255,0.1)]'
                }`}
        >
            <div className="flex items-center gap-4">
                {/* Toggle button */}
                <button onClick={handleToggle} disabled={toggling}
                    className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 transition-all duration-300 border
            ${habit.completed_today
                            ? 'bg-[rgba(52,211,153,0.12)] border-[rgba(52,211,153,0.25)] text-emerald-400'
                            : 'bg-[rgba(255,255,255,0.03)] border-[rgba(255,255,255,0.08)] text-[rgba(255,255,255,0.2)] hover:border-[rgba(201,178,124,0.25)] hover:text-[rgba(201,178,124,0.6)]'
                        }`}
                >
                    {habit.completed_today ? (
              ...
              ${habit.completed_today ? 'text-emerald-400/80 line-through' : 'text-[rgba(255,255,255,0.75)]'}`}>
                            {habit.name}
                        </p>
              ...
              text-[rgba(255,255,255,0.2)] hover:text-[rgba(255,255,255,0.5)] hover:bg-[rgba(255,255,255,0.05)]
              transition-all duration-200"
                    ...
                    {confirming ? (
                        <div className="flex items-center gap-1">
                            <button onClick={() => { onDelete(habit.id); setConfirming(false); }}
                                className="text-[10px] text-red-400 border border-red-400/30 px-2 py-1 rounded-lg hover:bg-red-400/10 transition-all">
                                Sí
                            </button>
                            <button onClick={() => setConfirming(false)}
                                className="text-[10px] text-[rgba(255,255,255,0.3)] border border-[rgba(255,255,255,0.08)] px-2 py-1 rounded-lg hover:bg-[rgba(255,255,255,0.05)] transition-all">
                                No
                            </button>
                        </div>
                    ) : (
...
const HistoryGrid = ({ history, onClose }) => {
    ...
    return (
        ...
        >
            <div className="rounded-2xl p-5 bg-[rgba(255,255,255,0.02)] border border-[rgba(255,255,255,0.06)] mt-3">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <button onClick={onClose}
                        className="w-6 h-6 rounded-lg flex items-center justify-center text-[rgba(255,255,255,0.2)] hover:text-[rgba(255,255,255,0.5)] transition-colors">
                        <X className="w-4 h-4" />
                    </button>
                </div>
                ...
                ${day.completed
                                    ...
                                    : 'bg-[rgba(255,255,255,0.03)] text-[rgba(255,255,255,0.1)]'
                                }`}
            ...
            <div className="rounded-2xl p-5 bg-[rgba(255,255,255,0.03)] border border-[rgba(201,178,124,0.15)] space-y-4">
                ...
                shadow-[0_16px_48px_rgba(0,0,0,0.5)] backdrop-blur-xl">
                    ...
                    <input
                        ...
                        className="flex-1 bg-transparent text-sm text-[rgba(255,255,255,0.7)] placeholder-[rgba(255,255,255,0.2)]
              border border-[rgba(255,255,255,0.08)] rounded-xl px-4 py-3
...
export default function HabitsPage() {
    ...
    const handleAdd = async (name, icon) => {
        try {
            await axios.post(`${API}/habits`, { name, icon }, { headers });
            await fetchHabits();
    ...
    const handleToggle = async (habitId) => {
        try {
            await axios.post(`${API}/habits/${habitId}/toggle`, {}, { headers });
            await fetchHabits();
    ...
    const handleDelete = async (habitId) => {
        try {
            await axios.delete(`${API}/habits/${habitId}`, { headers });
    ...
    const handleShowHistory = async (habitId) => {
        ...
        try {
            const res = await axios.get(`${API}/habits/${habitId}/history`, { headers });
            ...
            mb-4 transition-colors duration-200">
                ...
                {totalCount > 0 && (
                    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.5 }}
                        className="rounded-2xl p-5 bg-[rgba(255,255,255,0.025)] border border-[rgba(255,255,255,0.06)]">
                ...
                {loading ? (
                    ...
                    <div className="text-center py-16">
                        ...
                        <p className="text-sm text-[rgba(255,255,255,0.25)] mb-2">Aún no tienes hábitos</p>
